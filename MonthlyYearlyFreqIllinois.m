...
data = cellstr(data);
YearFreq = zeros(10,39);
MonthFreq = zeros(10,12);
...
title('Illinois Average Monthly Freezing Rain Events')
...
title('Illinois Average Yearly Freezing Rain Events')
legend('Yearly Average Freezing Rain')

% % Comparing low, mid, high frequency stations
% figure(4)
% plot(1:12,MonthFreq(14,1:12)/39,'k','linewidth',2)
% hold on 
% plot(1:12,MonthFreq(11,1:12)/39,'linewidth',2)
...
title('Illinois  Total Yearly Freezing Rain Events')
...
for i = 1:39 % (1976 - 2014)
    for x = 1:length(data) % Stations
        for j = 1:length(Month2) % Length of Year and Month matrices
            if Year2(x,j) == 1975+i
                if Month2(x,j) == 1
...
figure(11)
plot(1991:2004,edittotal(16:29),'k','linewidth',2)
xlabel('Year')
ylabel('Total Freezing Rain Events')
title('Edited Total Yearly Freezing Rain Events 1991-2004')
...
ylim([0 450])
legend('Edited Yearly Total Events')
...
title('Illinois Impact of Automated Measurements')
...
high_nov(1:39) = 0;
high_dec(1:39) = 0;
location = [3,4,5,7,10]; % Sites at latitude of interest

for i = 1:39 % (1976 - 2014)
    for x = 1:5 % Locations
        for j = 1:786 % Length of Year and Month matrices
...
mid_nov(1:39) = 0;
mid_dec(1:39) = 0;
location = [2,6,8,9,11]; % Sites at latitudes of interest
...
low_nov(1:39) = 0;
low_dec(1:39) = 0;
location = 1; % sites at latitudes of interest
...
xlabel('Year')
ylabel('Total Events/Year')
title('Illinois Low Latitude Stations','fontweight','bold','fontsize',18);
hold on
...
xlabel('Year')
ylabel('Total Events/Year')
title('Illinois Mid Latitude Stations','fontweight','bold','fontsize',18);
hold on
...
xlabel('Year')
ylabel('Total Events/Year')
title('Illinois High Latitude Stations','fontweight','bold','fontsize',18);
hold on

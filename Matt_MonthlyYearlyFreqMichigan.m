...
% Make a structure of all the filenames that end in ".txt":
filenames = dir('*.txt');
...
for i = 1:length(filenames)
    ...
    %dates(:,1:4) = cell2mat(Data(3:6));
    
    % Month2(i,1:length(Year)) = cell2mat(Data(2));
    % Month = cell2mat(Data(2));
    Year = cell2mat(Data(3));
    Month = cell2mat(Data(4));
    Year2(i,1:length(Year)) = cell2mat(Data(1));
    Hour = cell2mat(Data(6));
    
    %Record average wind speed and direction for each station:
    Dir = cell2mat(Data(8));
    WindDir(i) = mean(Dir(Dir >= 0));
    Speed = cell2mat(Data(9));
    WindSpeed(i) = mean(Speed(Speed >= 0));
    ...
    for k = 1:39
        ...
        for m = 1:24
            HourlyFreq_yearly(i,k,m) = numel(find(Hour(Year == (1975 + k)) == m - 1)); % dims are station, year, hour
        end
    end
    
    %Record hourly frequencies for hourly freq chart
    for k = 1:24
        HourlyFreq(i,k) = numel(find(Hour == (k - 1),length(Hour))); % Rows is Stations, Columns is Hours (1976-2014)
    end
...
for i = 1:39 % (1976 - 2014)
    for x = 1:length(filenames) % Stations
